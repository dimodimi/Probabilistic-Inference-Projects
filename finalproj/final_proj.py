...
def compute_empirical_distribution(values):
    ...
    #
    length = len(values)
    values_format = list(values)
    
    distribution = {item : values_format.count(item) / length for item in set(values_format)}
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def compute_empirical_mutual_info_nats(var1_values, var2_values):
    ...
    var12 = list(zip(var1_values, var2_values))
    p12 = compute_empirical_distribution(var12)
    
    
    empirical_mutual_info_nats = 0.0
                    
    for (x, y) in p12.keys():
        empirical_mutual_info_nats += p12[(x, y)] * np.log(p12[(x, y)] / (p1[x] * p2[y]))
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def chow_liu(observations):
    ...
    for i in range(num_vars):
        for j in range(i+1, num_vars):
            info[(i, j)] = compute_empirical_mutual_info_nats(observations[:,i], observations[:,j])
            
    sorted_info = sorted(info.items(), key = operator.itemgetter(1), reverse = True)
    
    for ((i, j), value) in sorted_info:
        if not (union_find.find(i) == union_find.find(j)):
...
def compute_empirical_conditional_distribution(var1_values, var2_values):
    ...
    #
    var12 = list(zip(var1_values, var2_values))
    var2 = list(var2_values)
    
    for x2 in set(var2_values):
        occ2 = var2.count(x2)
        for x1 in set(var1_values):
            conditional_distributions[x2][x1] = var12.count((x1, x2)) / occ2
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def learn_tree_parameters(observations, tree, root_node=0):
    ...
    for i in range(observations.shape[1]):
        if not (i == root_node):
            node_potentials[i] = {-1: 1, 0: 1, 1: 1}
        else:
            node_potentials[i] = compute_empirical_distribution(observations[:, root_node])
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
...
def sum_product(nodes, edges, node_potentials, edge_potentials):
    ...
    #Assign a value to root_node - can be an arbitary node - here the first element of nodes set
    for root_node in nodes:
        break
    
    #Make a list of (parent, child) edges - BFS - leaves at the end
    ...
    #From leaves to root
    rev_edges = list(edges_list)
    rev_edges.reverse()
    for (parent, child) in rev_edges:
        messages[(child, parent)] = {x_j: 0 for x_j in node_potentials[parent].keys()}
        for x_j in node_potentials[parent].keys():
            total = 0
            for x_i in node_potentials[child].keys():
                mid = node_potentials[child][x_i] * edge_potentials[(child, parent)][x_i][x_j]
                reduced_edge = list(edges[child])
                reduced_edge.remove(parent)
                for neighbor in reduced_edge:
                    mid *= messages[(neighbor, child)][x_i]
                total += mid
            messages[(child, parent)][x_j] = total
    #print(messages)

    #From root to leaves
    for (parent, child) in edges_list:
        messages[(parent, child)] = {x_j: 0 for x_j in node_potentials[child].keys()}
        for x_j in node_potentials[child].keys():
            total = 0
            for x_i in node_potentials[parent].keys():
                mid = node_potentials[parent][x_i] * edge_potentials[(parent, child)][x_i][x_j]
                reduced_edge = list(edges[parent])
                reduced_edge.remove(child)
                for neighbor in reduced_edge:
                    mid *= messages[(neighbor, parent)][x_i]
                total += mid
            messages[(parent, child)][x_j] = total
    
    for node in nodes:
        #marginals[node] = {x_i: 0 for x_i in node_potentials[node]}
        marginals[node] = {}
        total = 0

        for x_i in node_potentials[node].keys():
            mid = node_potentials[node][x_i]
            for neighbor in edges[node]:
                mid *= messages[(neighbor, node)][x_i]
            if mid != 0:
                marginals[node][x_i] = mid
            total += mid

        for x_i in marginals[node].keys():
...
def compute_marginals_given_observations(nodes, edges, node_potentials,
    ...
    for node in observations.keys():
        for key in new_node_potentials[node].keys():
            if key == observations[node]:
                new_node_potentials[node][key] = 1
            else:
                new_node_potentials[node][key] = 0
    #
    # END OF YOUR CODE
    # -------------------------------------------------------------------------
