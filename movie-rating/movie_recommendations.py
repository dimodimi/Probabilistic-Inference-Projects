...
def compute_posterior(prior, likelihood, y):
    ...
    for i in range(len(y)):
        log_sum += log_likelihood[y[i], :]
        
    num = log_prior + log_sum
    den = scipy.misc.logsumexp(num) 
...
def compute_movie_rating_likelihood(M):
    ...
    #
    r = np.array([np.arange(0, M)])
    c = np.transpose(r)
    
    likelihood = np.abs(r-c) + np.diag( 0.5 * np.array([1] * M) )
    likelihood = 1/likelihood
    
    norms = np.sum(likelihood, axis=0)
    
    for i in range(M):
        likelihood[:, i] /= norms[i]
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for movie in range(num_movies):
        if num_observations == -1:
            obs_ratings = movie_data_helper.get_ratings(movie)
        else:
            obs_ratings = movie_data_helper.get_ratings(movie)[:num_observations]
            
        posteriors[movie, :] = compute_posterior(prior, likelihood, obs_ratings)
...
def compute_entropy(distribution):
    ...
    entropy = -np.sum( scipy.special.xlogy(distribution, np.power(distribution, np.log2(np.e))) )
    #
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    dist, temp = infer_true_movie_ratings(num_observations)
    num_movies, M = dist.shape
    
    posterior_entropies = np.zeros(num_movies)    
    
    for movie in range(num_movies):
        posterior_entropies[movie] = compute_entropy(dist[movie, :])
    #
...
def main():
    ...
    x = np.array([1, 50, 100, 150, 200])
    y = np.zeros(5)
    for i in range(5):
        y[i] = np.mean(compute_true_movie_rating_posterior_entropies(x[i]))
